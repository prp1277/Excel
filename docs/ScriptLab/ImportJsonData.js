...
async function importJsonData() {
  await Excel.run(async (context) => {
    // Create a sheet for the dummy data
    await OfficeHelpers.ExcelUtilities.forceCreateSheet(context.workbook, "Sample");
    const sheet = context.workbook.worksheets.getItem("Sample");

    // Create a new table on the new sheet
    let expensesTable = sheet.tables.add("A1:E1", true);
    expensesTable.name = "ExpensesTable";
    expensesTable.getHeaderRowRange().values = [["postID", "id", "Name", "Email", "Body"]];

    // Use map function to iterate over the data
    const newData = transactions.map((item) => [item.postId, item.id, item.name, item.email, item.body]);
    ...
    // Activate the sheet and wait for the response
    sheet.activate();
    await context.sync();
  });
}
